...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  Quaternion<float> quatrn = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, yawEst);
  
  quatrn = quatrn.IntegrateBodyRate(gyro, dtIMU);
    
  float predictedPitch = quatrn.Pitch();
  float predictedRoll = quatrn.Roll();
  ekfState(6) = quatrn.Yaw();
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(5) = curState(5) + (accelerations.z - 9.81) * dt;
