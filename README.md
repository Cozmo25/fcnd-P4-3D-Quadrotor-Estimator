...
**Please find results and references to relevant code in-line with the scenarios.**

The relevant formulae for the coding implementations can be found in: [Estimation for Quadrotors](https://www.overleaf.com/read/vymfngphcccj) and in the Udacity FCND course material.
...
I imported the sensor data from `config/log/Graph1.txt` (GPS X Data) and `config/log/Graph2.txt` (Accelerometer X Data) to an excel spreadsheet and ran the built in standard deviation function on each dataset to determine the standard deviation of each sensor. This calculation can be found in [Graph1.xlsx](./src/config/log/Graph1.xlsx) and [Graph2.xlsx](./src/config/log/Graph2.xlsx)

The calculated standard deviations correctly match those found in `config/SimulatedSensors.txt` of 0.7 for the GPS sensor and 0.5 for the accelerometer.

Plugging those values into `config/06_SensorNoise.txt` passes the scenario:
```
...
Here I used the `FromEuler123_RPY` function of the supplied `Quarternion` class to create a Quaternion from the passed in estimated roll, pitch and yaw state values.

I then used the supplied `IntegrateBodyRate()` function to integrate the Quaternion values with the data from the IMU in the body frame.

Then we call the `Pitch()`, `Roll()` and `Yaw()` functions to generate predicted pitch, roll and yaw values in the Quaternion. Finally I normalised the predicted Yaw between -pi and pi.

The code for this implementation can be found in `src/QuadEstimatorEKF.cpp` on lines `100 - 108`.
...
4. In `QuadEstimatorEKF.cpp`, calculate the partial derivative of the body-to-global rotation matrix in the function `GetRbgPrime()`.  Once you have that function implemented, implement the rest of the prediction step (predict the state covariance forward) in `Predict()`.
...
The implementation of `PredictState()` can be found in `QuadEstimatorEKF.cpp` on lines `174 - 181`.

To determine the accelerations in the intertial frame from the body frame I passed the acceleration of the vehicle, in body frame (`accel`) to the supplied `Rotate_BtoI()` method from the `attitude` Quaternion. Once we have the accelerations in the inertial frame we can integrate them with the current state into an updated predicted state by multiplying by the timestep `dt`.

Running scenario 08_PredictState the estimated state tracks the actual state, with only reasonably slow drift, as shown in the figure above.
...
Finally, I completed the `Predict()` function using the supplied Jacobian `gPrime` of the transition function `g`. The code for this implementation can be found in `QuadEstimatorEKF.cpp` on lines `269 - 285`.
...
Retuning the `QPosXYStd` and the `QVelXYStd` parameters leads to us being able to capture the magnitude of the error we see.
...
Step 2 tuning the `QYawStd` parameter to 0.08 was found to approximately capture the magnitude of the drift.

The implementation of `UpdateFromMag()` can be found in `QuadEstimatorEKF.cpp` on lines `339 - 345`.
...
These variables are then passed to the `Update()` method of the `UpdateFromMag()` to perform the update.

After this implementation the following message is reported in the std output when the scenario is run:
...
The implementation of `UpdateFromGPS()` can be found in `QuadEstimatorEKF.cpp` on lines `310 - 317`.

Here we set the identity matrix of `hPrime` and then update `zFromX` state vector from the current `ekfState` vector and pass these variables to the `Update()` method.
...
After retuning the controller parameters by an approx 30% relaxation the output from the std output window is:
